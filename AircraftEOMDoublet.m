...
% Velocity Rates
u_e_dot = ((r .* v_e - q .* w_e) + (g .* -sin(theta)) + (X ./ aircraft_parameters.m));
v_e_dot = ((p .* w_e - r .* u_e) + (g .* cos(theta) .* sin(phi)) + (Y ./ aircraft_parameters.m));
w_e_dot = ((q .* u_e - p .* v_e) + (g .* cos(theta) .* cos(phi)) + (Z ./ aircraft_parameters.m));
